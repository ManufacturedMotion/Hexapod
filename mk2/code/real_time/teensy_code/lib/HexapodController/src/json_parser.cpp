...
void JsonParser::parseCommand(String command_str) {
    ...
    if (json_error) {
        _msg = "Error converting command to JSON object! Could not parse\nError is: ";
        _msg += json_error.c_str();
        _msg += "\n";        
    ...
    if (command_type == "PRE"){
        ...
        _Hexapod.serial.errorMsg("ERROR: Unsupported JSON command received via Serial\n");
    }
    return;
}
...
void JsonParser::performPreset(String preset) {
    ...
    if (preset == "Z") {
            ...
            _Hexapod.serial.logMsg("JSON parsing success; starfish preset selected (move all motors to zero).\n");
        #endif
        _Hexapod.moveToZeros();
    }
    else if (preset == "SIT"){
        #if LOG_LEVEL >= BASIC_DEBUG
            _Hexapod.serial.logMsg("JSON parsing success; sit preset selected.\n");
        #endif
        _Hexapod.sit();
    }
    else if (preset == "STND"){
        #if LOG_LEVEL >= BASIC_DEBUG
            _Hexapod.serial.logMsg("JSON parsing success; stand preset selected.\n");
        #endif
        _Hexapod.stand();
    }
    else {
        _msg = "ERROR: JSON parser detected input for a preset that is not yet supported: " + String(preset) + "\n"; 
        _Hexapod.serial.errorMsg(_msg);
    }
    return;
}
...
void JsonParser::performMovement(String movement) {
    ...
    if (movement == "RPD") {
        #if LOG_LEVEL >= BASIC_DEBUG
            _msg = "JSON rapid move parsing success; x, y, z is " + String (_x) + " " + String (_y) + " " + String (_z) + "\nroll, pitch, yaw, speed are " + 
        ...
        #if LOG_LEVEL >= BASIC_DEBUG
            _msg = "JSON walk setup parsing success; x, y, z is " + String (_x) + " " + String (_y) + " " + String (_z) + "\nroll, pitch, yaw, speed are " + 
                    String (_roll) + " " + String (_pitch) + " " + String (_yaw) + " " + String(_speed) + "\n";
            _Hexapod.serial.logMsg(_msg);
        #endif
    ...
    //NOTE: if not all 18 positions specified, motors who were not explicitly provided will move to where the hexapod has memory of them being. (What is in the parsers _leg_positions array)
        ...
        if (valid_MTPS){ 
            ...
            #if LOG_LEVEL >= BASIC_DEBUG
                _Hexapod.serial.logMsg("JSON move to pos at speed x18 parsing success.\n");
            #endif
            valid_MTPS = false;
        }
    }
        ...
        #if LOG_LEVEL >= BASIC_DEBUG
            _Hexapod.serial.logMsg("JSON leg enqueue parsing success.\n");
        #endif
    }
    else if (movement == "LMS") {
        #if LOG_LEVEL >= BASIC_DEBUG
            _msg = "JSON linear move setup parsing success; x, y, z is " + String (_x) + " " + String (_y) + " " + String (_z) + "\nroll, pitch, yaw, speed are " + 
                    String (_roll) + " " + String (_pitch) + " " + String (_yaw) + " " + String(_speed) + "\n";
            _Hexapod.serial.logMsg(_msg);
        #endif
        ...
        _msg = "ERROR: JSON parser detected input for a movement that is not yet supported: " + String(movement) + "\n";
        _Hexapod.serial.errorMsg(_msg);
    }
    return;
}
        ...
        if (key == "MV") {
            ...
            if (movement_sel == "TUNE") {
                if (json_command.size() != 2) {
                    _Hexapod.serial.errorMsg("ERROR: TUNE command only allows one position specification.\n");
                    return;
                }
                ...
                if (json_command.size() != 19) {
                    _Hexapod.serial.errorMsg("ERROR: MTPS command requires all 18 positions to be specified.\n");
                    valid_MTPS = false;
                    return;
                }
...
std::tuple<uint8_t, uint8_t, double> JsonParser::parseJsonJoint(String joint, JsonPair json_pos) {
    ...
    if (leg_start_idx == 0 || axis_start_idx == -1) {
        _Hexapod.serial.errorMsg("ERROR: Joint key requires the form 'L<leg>S<axis>'.\n");
        return error_return;
    }
    if (axis_start_idx == -1 || axis_start_idx == int(joint.length() - 1)) {
        _Hexapod.serial.errorMsg("ERROR: Joint key requires a valid 'S' value after 'L<leg>S<axis>'.\n");
        return error_return;
    }
